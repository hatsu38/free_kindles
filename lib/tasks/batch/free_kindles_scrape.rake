  ...
  task free_kindles_scrape: :environment do
    ...
    [*START_PAGE..MAX_PAGE].each do |page|
      ...
      blocks.each do |block|
        ...
        book_series_object = BookSeries.new(title: book_series_title, books_count: series_books_count, amazon_url: book_series_amazon_url)
        book_series_object.books.build(title: title, book_number: book_number, price: price, amazon_url: link, amazon_image_url: image_url)
        insert_book_serieses << book_series_object
      end
    end
    BookSeries.import insert_book_serieses, recursive: true
  end
end
